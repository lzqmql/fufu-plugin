...
const bq = {}
try {
  const res = JSON.parse(fs.readFileSync("plugins/fufu-plugin/resources/bq.json"))
...
const url = "https://bqb.fufuidc.com/memes/"

// 定义最大重试次数和每次重试的间隔
const MAX_RETRIES = 3
const RETRY_DELAY = 2000  // 单位：毫秒
...
export class dailyNoteByWidget extends plugin {
  ...
  async bqlb(e) {
    try {
      const res = await fetchWithRetry(`${url}render_list`, { method: "POST" })
      const resultBuffer = Buffer.from(await res.arrayBuffer())
      return e.reply(segment.image(resultBuffer))
    } catch (error) {
      return e.reply(`获取表情列表失败: ${error.message}`)
    }
  }
  ...
  async accept(e) {
    ...
    try {
      const res = await fetchWithRetry(`${url}${item.key}/`, { method: "POST", body: formData })
      if (res.status > 299)
        return e.reply(`该表情至少需要${item.params_type.min_images}张图片，${item.params_type.min_texts}个文字描述`, true)

      const resultBuffer = Buffer.from(await res.arrayBuffer())
      return e.reply(segment.image(resultBuffer))
    } catch (error) {
      return e.reply(`请求失败: ${error.message}`)
    }
  }
}
